@extends('layout')

@section('content')

<div class="row">
  <div class="col-xs-2">
    <p>メニューメニューメニュー</p>
    <ul class="menuuu">
    </ul>
  </div>
  <div class="col-xs-10">
    <p>Hello maps!</p>

    <!-- 地図を表示する -->
    <div id="map" style="width: 90%; height: 500px"></div>
    <script>
      //var marker;
      var markers = Array();
      var currentInfoWindow;
      function initMap(){

        var tokyo = {lat: 35.6808586, lng: 139.7669568};
        var map = new google.maps.Map(document.getElementById('map'), {
          zoom: 15,
          center: tokyo
        });

        google.maps.event.addListener(map, 'idle', function(){
          setPointMarker();
        });

        function newMark(response){
            var i = markers.length;
            // console.log(i);
            markers[i] = new google.maps.Marker({
              position: {lat: response['lat'], lng: response['lng']},
              map: map,
              //icon: "{{ asset('assets/img/small.png') }}"
              //icon: "http://localhost:8000/assets/favicon.png"
          });
          //吹き出し表示
          var infoWnd = new google.maps.InfoWindow({
            content: response['name'] + "<p>あいうえお<p>",
          });
          //マーカクリック時の処理
          google.maps.event.addListener(markers[i], "click", function(){
                //markers[i].setIcon("{{asset('assets/img/big.png')}}");
                //先に開いた情報ウィンドウがあれば、closeする iconも戻す
                if (currentInfoWindow) {
                  currentInfoWindow.close();
                  //nowM.setIcon( "{{asset('assets/img/small.png')}}" );
                }
                //情報ウィンドウを開く
                infoWnd.open(map, markers[i]);
                //開いた情報ウィンドウを記録しておく マーカも記録
                currentInfoWindow = infoWnd;
                nowM = markers[i];
          });
        }

        onload=function(){
          $(function(){
              $('li').click(function(){
                  var index = $(this).index();
                  console.log('要素:' + index + '番目');
                  google.maps.event.trigger(markers[index], "click");
              });
          });
        }

        function setPointMarker(){
          //古いマーカを削除
          if(markers.length > 0){
            for (i = 0; i <  markers.length; i++) {
              markers[i].setMap(null);
              // console.log("delete");
            }
            //配列削除
              markers = [];
            //リスト削除
            $('ul.menuuu').empty();
          }
          //地図の東西南北境界取得
          var bounds = map.getBounds();
          var map_latlng = {map_ne_lat:bounds.getNorthEast().lat(),
                            map_sw_lat:bounds.getSouthWest().lat(),
                            map_ne_lng:bounds.getNorthEast().lng(),
                            map_sw_lng:bounds.getSouthWest().lng()
                            };

          $.ajax({
            url: '/mapajax',
            type: 'GET',
            dataType: 'json',
            timeout: 1000,
            data: map_latlng
          }).done(function(responseData) {
            var Mnum = 0;
            responseData.forEach(function(response){
              newMark(response);
              //ここでリスト表示したい
              ShopList = $('ul.menuuu').append("<li id=" + Mnum + "><a href=\"javascript:void(0)\">" + response['shop_name'] + "</a></li>");
              Mnum = Mnum + 1;
            });
          }).fail(function(error) {
            console.log(error);
            console.log('nnn');
          });
        }
      }

    </script>

  </div><!-- map -->
</div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
<script
async defer
src ="https://maps.googleapis.com/maps/api/js?key=AIzaSyAvoocj2Tn6Kc08gKXjZT5sUYfJ1KPqOwg&callback=initMap">
</script>


@endsection